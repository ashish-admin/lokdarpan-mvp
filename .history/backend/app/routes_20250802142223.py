...
# --- New Import for AI Analysis ---
from .services import analyze_emotions # We will reuse this for the AI connection
...
# --- NEW STRATEGIC SUMMARY ENDPOINT ---
...
def strategic_summary():
    ...
    try:
        ...
        top_positive = emotion_counts[emotion_counts.index.isin(positive_emotions)].idxmax()
        top_negative = emotion_counts[emotion_counts.index.isin(negative_emotions)].idxmax()

        # Get example text for context
        ...
        prompt = f"""
        As a political strategist, analyze the following sentiment data from public discourse.
        Provide a brief, actionable strategic summary. Your summary must have two parts: "Opportunity" and "Threat".
        ...
        # 3. Call the AI service (reusing the model from services.py)
...
def analyze_emotions_for_summary(prompt):
    # This is a helper function to call the Gemini API, adapted from services.py
